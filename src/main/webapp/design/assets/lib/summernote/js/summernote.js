...
// CONCATENATED MODULE: ./src/js/base/core/range.js
                ...
                function () {
                    ...
                    range_createClass(WrappedRange, [{
                        ...
                        value: function select() {
                            ...
                            if (env.isW3CRangeSupport) {
                                var sdesign = document.getSdesign();

                                if (sdesign.rangeCount > 0) {
                                    sdesign.removeAllRanges();
                                }

                                sdesign.addRange(nativeRng);
                            } else {
                                nativeRng.select();
                            }
            ...
            /* harmony default export */ var range = ({
                /**
                 * create Range Object From arguments or Browser Sdesign
                 *
                 * @param {Node} sc - start container
                 * @param {Number} so - start offset
                 * @param {Node} ec - end container
                ...
                create: function create(sc, so, ec, eo) {
                    if (arguments.length === 4) {
                        ...
                        var wrappedRange = this.createFromSdesign();
                ...
                createFromSdesign: function createFromSdesign() {
                    var sc, so, ec, eo;

                    if (env.isW3CRangeSupport) {
                        var sdesign = document.getSdesign();

                        if (!sdesign || sdesign.rangeCount === 0) {
                            return null;
                        } else if (dom.isBody(sdesign.anchorNode)) {
                            // Firefox: returns entire body as range on initialization.
                            // We won't never need it.
                            return null;
                        }

                        var nativeRng = sdesign.getRangeAt(0);
                        ...
                        // IE8: TextRange
                        var textRange = document.sdesign.createRange();
    ...
    */
                        ...
                        value: function fontStyling(target, value) {
                            ...
                            if (rng !== '') {
                                ...
                                this.$editor.find('.note-status-output').html('<div id="note-status-output-' + noteStatusOutput + '" class="alert alert-info">' + this.lang.output.noSdesign + '</div>');
                                setTimeout(function () {
                ...
                function () {
                    ...
                    Handle_createClass(Handle, [{
                        ...
                        value: function initialize() {
                            ...
                            this.$handle = external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()(['<div class="note-handle">', '<div class="note-control-sdesign">', '<div class="note-control-sdesign-bg"></div>', '<div class="note-control-holder note-control-nw"></div>', '<div class="note-control-holder note-control-ne"></div>', '<div class="note-control-holder note-control-sw"></div>', '<div class="', this.options.disableResizeImage ? 'note-control-holder' : 'note-control-sizing', ' note-control-se"></div>', this.options.disableResizeImage ? '' : '<div class="note-control-sdesign-info"></div>', '</div>', '</div>'].join('')).prependTo(this.$editingArea);
                            this.$handle.on('mousedown', function (event) {
                                if (dom.isControlSizing(event.target)) {
                                    event.preventDefault();
                                    event.stopPropagation();

                                    var $target = _this2.$handle.find('.note-control-sdesign').data('target');
                        ...
                        value: function update(target, event) {
                            ...
                            var $sdesign = this.$handle.find('.note-control-sdesign');
                            ...
                            if (isImage) {
                                ...
                                $sdesign.find('.note-control-sdesign-info').text(sizingText);
