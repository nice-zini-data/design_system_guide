...
const login_nav_list = () => {
    ...
    $('.myNavListBox, .notice').click(function(){
        ...
        if($(this).find('.myListArrow').hasClass('active')){
            $('.myNavListBox > img').attr('src','/design/assets/design/images/icon/perm_identity_gr.svg');

        }else{
            $('.myNavListBox > img').attr('src','/design/assets/design/images/icon/perm_identity.svg')
        }

        if($('.notice').hasClass('active')){
            $('.notice > img').attr('src','/design/assets/design/images/icon/notifications_at_2.svg');

            if($(this).hasClass('on')){
                $('.notice > img').attr('src','/design/assets/design/images/icon/notifications_active.svg');
            }

        }else{
            $('.notice > img').attr('src','/design/assets/design/images/icon/notifications.svg');
            
            if($(this).hasClass('on')){
                $('.notice > img').attr('src','/design/assets/design/images/icon/notifications_on.svg');
            }else{
                $('.notice > img').attr('src','/design/assets/design/images/icon/notifications.svg');
            }
