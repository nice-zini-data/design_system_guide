...
function getAjaxCross(dst_id, dst_url, dst_params, successFunc, errorFunc, option, async, loading) {


  $.ajax({
    beforeSend : function(xhr){
      xhr.setRequestHeader("Access-Control-Allow-Origin","*");
      xhr.setRequestHeader('Access-Control-Allow-origin', 'https://deveatout.nicebizmap.co.kr');
      xhr.setRequestHeader('Access-Control-Allow-Credentials', 'true');
    },
    header: {
      'Access-Control-Allow-Origin' : 'https://deveatout.nicebizmap.co.kr',
      'Access-Control-Allow-Credentials' : 'true',
    },
    type: isEmpty(option) ? 'POST' : option,
    url: dst_url,
    dataType: 'json',
    data: dst_params,
...
function getAjax(dst_id, dst_url, dst_params, successFunc, errorFunc, option, async, loading) {
  // 파라미터에 memNo 없을때 세션에 가지고 있던 memNo 넣기
  (common.isEmpty(dst_params.memNo)) ? dst_params.memNo = Number(sessionStorage.getItem("memNo")) : "";
