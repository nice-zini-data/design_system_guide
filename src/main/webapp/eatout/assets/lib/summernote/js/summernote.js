...
// CONCATENATED MODULE: ./src/js/base/core/range.js
                ...
                function () {
                    ...
                    range_createClass(WrappedRange, [{
                        ...
                        value: function select() {
                            ...
                            if (env.isW3CRangeSupport) {
                                var seatout = document.getSeatout();

                                if (seatout.rangeCount > 0) {
                                    seatout.removeAllRanges();
                                }

                                seatout.addRange(nativeRng);
                            } else {
                                nativeRng.select();
                            }
            ...
            /* harmony default export */ var range = ({
                /**
                 * create Range Object From arguments or Browser Seatout
                 *
                 * @param {Node} sc - start container
                 * @param {Number} so - start offset
                 * @param {Node} ec - end container
                ...
                create: function create(sc, so, ec, eo) {
                    if (arguments.length === 4) {
                        ...
                        var wrappedRange = this.createFromSeatout();
                ...
                createFromSeatout: function createFromSeatout() {
                    var sc, so, ec, eo;

                    if (env.isW3CRangeSupport) {
                        var seatout = document.getSeatout();

                        if (!seatout || seatout.rangeCount === 0) {
                            return null;
                        } else if (dom.isBody(seatout.anchorNode)) {
                            // Firefox: returns entire body as range on initialization.
                            // We won't never need it.
                            return null;
                        }

                        var nativeRng = seatout.getRangeAt(0);
                        ...
                        // IE8: TextRange
                        var textRange = document.seatout.createRange();
    ...
    */
                        ...
                        value: function fontStyling(target, value) {
                            ...
                            if (rng !== '') {
                                ...
                                this.$editor.find('.note-status-output').html('<div id="note-status-output-' + noteStatusOutput + '" class="alert alert-info">' + this.lang.output.noSeatout + '</div>');
                                setTimeout(function () {
                ...
                function () {
                    ...
                    Handle_createClass(Handle, [{
                        ...
                        value: function initialize() {
                            ...
                            this.$handle = external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()(['<div class="note-handle">', '<div class="note-control-seatout">', '<div class="note-control-seatout-bg"></div>', '<div class="note-control-holder note-control-nw"></div>', '<div class="note-control-holder note-control-ne"></div>', '<div class="note-control-holder note-control-sw"></div>', '<div class="', this.options.disableResizeImage ? 'note-control-holder' : 'note-control-sizing', ' note-control-se"></div>', this.options.disableResizeImage ? '' : '<div class="note-control-seatout-info"></div>', '</div>', '</div>'].join('')).prependTo(this.$editingArea);
                            this.$handle.on('mousedown', function (event) {
                                if (dom.isControlSizing(event.target)) {
                                    event.preventDefault();
                                    event.stopPropagation();

                                    var $target = _this2.$handle.find('.note-control-seatout').data('target');
                        ...
                        value: function update(target, event) {
                            ...
                            var $seatout = this.$handle.find('.note-control-seatout');
                            ...
                            if (isImage) {
                                ...
                                $seatout.find('.note-control-seatout-info').text(sizingText);
...
// CONCATENATED MODULE: ./src/js/bs3/ui.js
            ...
            var dropdown = renderer["a" /* default */].create('<ul class="note-dropdown-menu dropdown-menu">', function ($node, options) {
                var markup = Array.isArray(options.items) ? options.items.map(function (item) {
                    ...
                    return '<li aria-label="' + value + '"><a href="javascript:;" ' + (dataValue + dataOption) + '>' + content + '</a></li>';
                }).join('') : options.items;
            ...
            var dropdownCheck = renderer["a" /* default */].create('<ul class="note-dropdown-menu dropdown-menu note-check">', function ($node, options) {
                var markup = Array.isArray(options.items) ? options.items.map(function (item) {
                    var value = typeof item === 'string' ? item : item.value || '';
                    var content = options.template ? options.template(item) : item;
                    return '<li aria-label="' + item + '"><a href="javascript:;" data-value="' + value + '">' + icon(options.checkClassName) + ' ' + content + '</a></li>';
                }).join('') : options.items;
                $node.html(markup).attr({
                    'aria-label': options.title
                });
