...
<script>
	document.title = "Kingmaker 아이디 찾기";
</script>
	...
	<div class="login_page">
		<div class="login_box">
			...
			<div class="changePwTxt">
				...
				<span class="txt_n_sb">아이디 찾기</span>
				<p class="chip_txt_s_m">가입시 입력하신 메일 계정으로 아이디가 전송됩니다.</p>
			</div>
			...
			<div class="login_btn">
				<button type="button" class="pri_defaultBtn txt_l_sb">아이디 찾기</button>
			</div>
		</div>
	</div>
	...
	$(function() {
		...
		$(".pri_defaultBtn").on('click', function (){
			if(isValid()){
				...
				getAjax("getLoginId", "/common/getLoginId", data, function (id, response){
					if(response.code == "C005"){
						alert('아이디 찾기 메일을 발송했습니다.');
						location.href = "/login";
